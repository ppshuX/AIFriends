...
1. 生成 `backend/.env`（随机签名密钥 + 默认配置）；
2. 创建 SQLite 数据库并导入 4 个官方演示角色与 4 个腾讯云音色；
3. 启动本地服务并打开浏览器。

按 `Ctrl+C` 或直接关闭窗口即可停止服务。数据保存在 `backend/db.sqlite3`、
`backend/media/` 与 `backend/web/documents/` 中，删除整个文件夹即可完全卸载。
...
这是预期行为。如需完整体验，用记事本编辑 `backend/.env`：
